 ...
 * 
 * _Tracer 1.1 Sep 09, 2007
 */
package com.cg.utils {
	import flash.net.LocalConnection;
	import flash.events.StatusEvent;
	
	public class RemotingTracer extends Object{
		...
		public static function set DoNotBroadcast(val:Boolean):void{
			__Flash_Tracer_DoNotBroadcast = val;
		}
		...
		public static function str(...arguments):String {
			var strValue:String = arguments.join(" | ");
			if(!__Flash_Tracer_DoNotTrace){
				trace(strValue);
			}
			if(!__Flash_Tracer_DoNotBroadcast){
				dispatch(strValue);
			}
			return strValue;
		}
		...
		public static function lev(...arguments):String{
			...
			var levels:Number = (arguments.length > 1)?Number(arguments[1]):_Tracer.MAX_NUMBER_LEVELS;
			var titleVal:String = "";
			if(isNaN(levels)){
				titleVal = arguments[1];
				levels = _Tracer.MAX_NUMBER_LEVELS;
			}
			titleVal += (arguments.length > 2)?arguments[2]:"";
			for(var i:Number = 3; i < arguments.length; i++) titleVal += " | " + arguments[i];

			var tr:_Tracer = new _Tracer(objVal,titleVal,__Flash_Tracer_DoNotTrace,levels);
		...
		public static function obj(...arguments):String {
			...
			var tr:_Tracer = new _Tracer(objVal,titleVal,__Flash_Tracer_DoNotTrace, _Tracer.MAX_NUMBER_LEVELS);
			var strData:String = tr.getInfo();
			if(!__Flash_Tracer_DoNotBroadcast){
				dispatch(strData);
			}
		...
		private static function dispatch(strData:String):void{
			localConnection.send("__Flash_Tracer", "Add", strData);
		}
...
class _Tracer extends Object{
	...
	public function _Tracer(object:Object, title:String = "", supressTrace:Boolean = false, allowedDepth:Number =  _Tracer.MAX_NUMBER_LEVELS){
		this._object = object;
		this._title = title;
	...
	private function parseLiteral(object:Object, name:String, isInsideCollection:Boolean = false):void{
		...
		else this.appendString(this.generatePropertyName(name, isInsideCollection)+_Tracer.objectToString(object));
	}
	...
	private function getIndent():String{
		if(_Tracer.indentCache[this._indent] == undefined){
			var sb:StringBuilder = new StringBuilder();
			for(var i:uint = 0; i < this._indent; i++){
				sb.append(_Tracer.INDENT_CHAR);
			}
			_Tracer.indentCache[this._indent] = "\n"+sb.toString();
		}
		var isSibling = (this._prevIndent == this._indent);
		this._prevIndent = this._indent;
		return ((isSibling)?",":"")+_Tracer.indentCache[this._indent];
	}
