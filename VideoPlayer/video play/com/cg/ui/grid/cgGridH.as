package com.cg.ui.grid 
{
	 ...
	 * @author cg
	 */
	public class cgGridH extends UI 
	{
		...
		//是否根据cell宽度自动调整列宽及间隔
		protected var _auto_row:Boolean = false;
		protected var _old_row:int = 1;
		protected var _drow:int = 0;
		protected var _grid_height:int = 0;
		...
		protected function setPos():void
		{
			...
			if (_auto_row) {
				...
				for (i = 0; i < cells.length; i++) {
					...
					if (__y + cells[i].height > _grid_height) {
						//换行 
						__y = 0;
						__x += _max_x + _border_x;
						_max_x = 0;
					...
					cells[i].y = __y;
					_cells.push(cells[i]);
					__y += cells[i].height + _border_y;
					_max_x = Math.max(_max_x, cells[i].width);
				}
			}else{
				for (i = 0; i < cells.length; i++) {
					old_pos[i] = new Point(cells[i].x, cells[i].y);
					cells[i].x = Math.floor(i / _row) * (_cell_width + _border_x);
					cells[i].y = i % _row * (_cell_height + _border_y);
				}
			}
		}
		/**
		 * 根据grid设置的宽度对齐每一列的cell
		 * @param	rowCells
		 */
		protected function setAutoBorder(rowCells:Array):void
		{
			...
			for (i = 0; i < rowCells.length; i++) {
				_size += rowCells[i].height;
			}
			var _bd:Number = (_grid_height-_size) / (rowCells.length - 1);
			var __y:Number = 0;
			for (i = 0; i < rowCells.length; i++) {
				rowCells[i].y = __y;
				__y += rowCells[i].height + _bd;
			}
		}
		protected function onTweenUpdate():void
		{
			...
			for (var i:int = 0; i < cells.length; i++) {
				var __x:Number = Math.floor(i / (_old_row +_drow * tween_obj.t));
				var __y:Number = i % (_old_row +_drow * tween_obj.t);
		...
		public function get grid_height():int 
		{
			return _grid_height;
		}
		
		public function set grid_height(value:int):void 
		{
			_grid_height = value;
			if (_grid_height > 0) {
				var _num:int
				if(_auto_border && _auto_row==false){
					_num = Math.max(1, Math.floor(_grid_height / _cell_height));
					if (_num == 1) {
						_border_y = _border_def_y;
					}else {
						_border_y = (_grid_height - _cell_height * _num) / (_num - 1);
						_border_y = Math.max(0, _border_y);
					}
					if (_border_equals && _border_y>0) {
						_border_x = _border_y;
					}
					update();
				}else {
					_num = Math.max(1, Math.floor((_grid_height+_border_y) / (_cell_height+_border_y)));
				}
				if (_row != _num) {
					row = _num;
				}else if(_auto_row) {
